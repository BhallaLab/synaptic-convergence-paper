...
params = {
    ...
    'thetaFreq':8.0, 		# 
    'amparSynapseWeight': 20.0,    #
    'nmdarSynapseWeight': 20.0,   #
    'gabarSynapseWeight': 20.0,    #
...
moogList = [
    ...
    ['#', '1', 'DEND/CaM/CaM_Ca4', 'conc', '[CaMCa4] (uM)',0, 0.002],
...
def runModel( params, args ):
    rdes = buildModel( params, args )
    t0 = time.time()
    moose.reinit()
    zeroGluVec()

    rdes.displayMoogli( 0.01, args.runtime, 0.0, colormap = 'plasma', mergeDisplays = True, bg = 'default' )
...
def buildModel( params, args ):
    ...
    rdes = rd.rdesigneur(
        ...
        stimList = [
                ...
                [ 'head#',  str( params['amparSynapseWeight'] ), 'glu', 'periodicsyn', "0.001"],
                [ 'head#',  str( params['nmdarSynapseWeight'] ), 'NMDA', 'periodicsyn', "0.001"],
            ],
        adaptorList = [
...
def main():
    ...
    if not args.outputFile:
        args.outputFile = 'out7_frq{}_Dt{}_Dx{}_N{}_{}_p{}.h5'.format( 
                args.frequency, args.seqDt, args.seqDx, 
                args.seqN, "grp" if args.groupStimulus else "seq",
                args.permutation )
    # for the purposes of display, we'll deliver a spaced Ca stimulus
    args.frequency = 10
    args.seqDt = 1
    args.seqDx = 5
    args.seqN = 5
    args.groupStimulus = True
    args.runtime = 2
